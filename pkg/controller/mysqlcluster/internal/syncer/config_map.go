...
func buildMysqlConfData(cluster *mysqlcluster.MysqlCluster) (string, error) {
        ...
        addKVConfigsToSection(sec, getMysqlMasterSlave5Configs(), cluster.Spec.MysqlConf)
        } else if cluster.GetMySQLSemVer().Major == 8 {
        addKVConfigsToSection(sec, getMysqlMasterSlave8Configs(), cluster.Spec.MysqlConf)
        }
...
func getMysqlMasterSlave5Configs() map[string]intstr.IntOrString {
        config := make(map[string]intstr.IntOrString)

        for key, value := range mysqlMasterSlave5Configs {
...
func getMysqlMasterSlave8Configs() map[string]intstr.IntOrString {
        config := make(map[string]intstr.IntOrString)

        for key, value := range mysqlMasterSlave8Configs {
                config[key] = intstr.Parse(value)
        }
...
// mysqlMasterSlave5Configs represents the configuration that mysql-operator needs by default
var mysqlMasterSlave5Configs = map[string]string{
...
// mysqlMasterSlave8Configs represents the configuration that mysql-operator needs by default
var mysqlMasterSlave8Configs = map[string]string{
        "log-bin":           "/var/lib/mysql/mysql-bin",
        "log-slave-updates": "on",
